...
export class PhotoService {
  ...
  getPhotos() {
    let url = "http://localhost:8080/api/photo/allPhotosList";
    return this.http.get(url);
  }
   getPhotoById (photoId: number) {
    let tokenUrl1 = "http://localhost:8080/api/rest/photo/photoId";
 ...
 getPhotosByUser (user: User) {
   let tokenUrl1 = "http://localhost:8080/api/rest/photo/user";
    let headers1 = new Headers({'Content-Type': 'application/json', 'Authorization': 'Bearer '+localStorage.getItem("token")});
     return this.http.post(tokenUrl1, JSON.stringify(user), {headers: headers1});
   }

   updatePhoto(photo: Photo) {
    let tokenUrl1 = "http://localhost:8080/api/rest/photo/update";
    let headers1 = new Headers({'Content-Type': 'application/json', 'Authorization': 'Bearer '+localStorage.getItem("token")});
